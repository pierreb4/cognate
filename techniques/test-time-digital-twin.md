...
caveats:
  ...
  - "Edges still OWED, not declared. `modeling.belief-update` (counterexample-guided repair of the twin) and `exploration.experiment-design` (the 'live wall' picks what the next action should teach, but by one held hypothesis, not by information gain over a set) have no written strength ladder to grade against. The `modeling.hypothesis-formation` edge is declared: the pair was typed `overlaps` against all eight techniques at direct/partial there, each entry on the alphabetically-first node, so none of those entries lives on this node; the one entry here is the goal-inference pair with `technique.theory-based-rl`, which sorts after it."
  - "The public 25-game set is saturated and Twin's 93.3 is not its top. ARC Prize's community leaderboard (https://arcprize.org/leaderboard/community, read 2026-09-06; its own rule: 'Scores are self-reported unless noted otherwise. Results on the ARC-AGI-1 and ARC-AGI-2 semi-private sets are run and verified by ARC Prize. Everything else is scored on a public set and self-reported.') lists on ARC-AGI-3 Public Demo: Tycho 100.0% ($2,986, 2026-07-29, https://arxiv.org/abs/2607.28287 — 'can delegate a falsification-tested executable world model to a builder for planning'), Retrodict 99.9% ($654, 2026-07-19, https://github.com/ryanbbrown/Retrodict — 'requires rule hypotheses to retrodict recorded history before spending live actions'), baseline1 99.0% ($400, 2026-07-15, https://arxiv.org/abs/2605.05138 — 'Coding agent that builds and verifies an executable Python world model, then plans through it'), OPINE-World 78.4% ($1,040, 2026-07-01, https://arxiv.org/abs/2607.01531). NVIDIA's AVO post (https://developer.nvidia.com/blog/nvidia-avo-reaches-100-on-arc-agi-3-demonstrating-a-frontier-level-general-purpose-architecture-for-long-horizon-autonomous-agents/, 2026-08-21) reports 100 RHAE on all 25 public environments with a general coding agent that, by its own account, builds no programmatic world model. None of these is an evidence row here: the board is self-reported, a one-line description does not establish the replay-every-logged-transition constraint that defines this node, and a world-model-free agent reaching the same ceiling on the public set undercuts any claim that the twin mechanism carries the public-set result — the public 25 games no longer separate mechanisms."
  - "Verified semi-private ARC-AGI-3 numbers exist and none is a twin result; they are model results and enter here only as the comparator the semi-private column would hold. ARC Prize: GPT-6 Astra 'was 62.7% at max reasoning for $26,098' with the Standard harness and with the Provider Adapter harness 'its best observed result was 99.9% at high reasoning for $18,817' (https://arcprize.org/results/openai-gpt-6-astra, 2026-09-02); 'As of July 24, 2026, Claude Opus 5 (High) is the highest-performing model on ARC-AGI-3, scoring 30.2%.' (https://arcprize.org/results/anthropic-claude-opus-5); GPT-5.6 Sol at max 'averaging 13.33% on Public and 7.78% on Semi-Private' (https://arcprize.org/results/openai-gpt-5-6-sol, 2026-07-09). Twin publishes no semi-private number, so the 93.3 vs 62.7 / 99.9 comparison is not split-matched."
  - "The ARC-AGI-3 metric changed on 2026-04-14 and public numbers before and after are not on one scale. ARC Prize: 'Technical change: the human baseline which normalizes scores moves from 2nd-best player to median player per level.' and 'Technical change: per-level score cap increases from 100% to 115%.' with 'The net result of these changes is a marginal increase in scores for both humans and AI (+0.5pp)' (https://arcprize.org/blog/arc-agi-3-human-dataset). Twin's rows (2026-08-14) are on the new metric; launch-era public numbers (Symbolica's launch-week 36.08%, https://www.symbolica.ai/blog/arc-agi-3; the 0.51% frontier baseline, https://arcprize.org/blog/arc-agi-3-launch) are on the old one, and the Kaggle track's public leaderboard has run under the new rule since the change."
interacts:
  - technique: technique.theory-based-rl
    rel: overlaps
    scope: goal-setting.goal-inference
    note: >-
      both hold a nameable objective that play refutes, from the same sparse terminal signal;
      the twin writes a free-form predicate for a state five heuristics ranked, EMPA picks a
      class and a polarity inside one authored template — a ranking prior against a
      hypothesis space, which is why both are `partial` and EMPA's cap is the heavier
provenance:
  entered: 2026-09-05
  commit: 90bb75b
